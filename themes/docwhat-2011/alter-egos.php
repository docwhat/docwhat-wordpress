<ul>
  <li>
    <h4>I.M. Egos</h4>
    <ul>
      <li>
        <a rel="me"
           href="http://freenode.net/">freenode</a>: docwhat
      </li>
      <li>
        <a rel="me"
           href="http://gmail.com/">GMail</a>: docwhat
      </li>
      <li>
        <a rel="me"
           href="http://www.aim.com/">AIM</a>: <a href="aim:goim?screenname=TheDoctorWhat">TheDoctorWhat</a>
      </li>
      <li>
        <a rel="me"
           href="http://www.icq.com/">ICQ</a>: 109035922
      </li>
      <li>
        <a rel="me"
           href="http://irc.mozilla.org/">Mozilla</a>: docwhat
      </li>
      <li>
        <a rel="me"
           href="http://messenger.yahoo.com/">Yahoo</a>: TheDoctorWhat
      </li>
    </ul>
  </li>
  <li>
    <h4>Social Egos</h4>
    <ul>
      <li>
        <a rel="me"
           href="https://twitter.com/docwhat">Twitter</a>
      </li>
      <li>
        <a rel="me"
           href="http://www.facebook.com/docwhat/">Facebook</a>
      </li>
      <li>
        <a rel="me"
           href="http://www.flickr.com/photos/docwhat/">Flickr</a>
      </li>
      <li>
        <a rel="me"
           href="http://www.linkedin.com/in/docwhat">LinkedIn</a>
      </li>
      <li>
        <a rel="me"
           href="http://www.advogato.org/person/docwhat/">Advogato</a>
      </li>
      <li>
        <a rel="me"
           href="http://boardgamegeek.com/user/docwhat">BoardGameGeek</a>
      </li>
      <li>
        <a rel="me"
           href="http://del.icio.us/docwhat/">Del.icio.us</a>
      </li>
      <li>
        <a rel="me"
           href="https://plus.google.com/+ChristianHoltje/about">Google+</a>
      </li>
      <li>
        <a rel="me"
           href="http://osx.iusethis.com/user/docwhat">i use this</a>
      </li>
      <li>
        <a rel="me"
           href="http://www.last.fm/user/docwhat/">Last.FM</a>
      </li>
      <li>
        <a rel="me"
           href="http://musicbrainz.org/show/user/?username=docwhat">MusicBrainz</a>
      </li>
      <li>
        <a rel="me"
           href="http://slashdot.org/~docwhat/">Slashdot</a>
      </li>
      <li>
        <a rel="me"
           href="http://www.spore.com/view/profile/docwhat">Spore</a>
      </li>
      <li>
        <a rel="me"
           href="https://www.youtube.com/user/TheDoctorWhat">YouTube</a>: TheDoctorWhat
      </li>
    </ul>
  </li>
  <li>
    <h4>Gaming Egos</h4>
    <ul>
      <li>
        <a rel="me" href="http://steamcommunity.com/id/docwhat/">Steam</a>: docwhat
      </li>
      <li>
        <a rel="me"
           href="http://battle.net/">Battle.Net</a>: docwhat#1698
      </li>
      <li>
        <a title="3DS Friend Code"
           href="http://www.wikihow.com/Enter-Nintendo-3DS-Friend-Codes">3DS</a>: 3496-9806-5664
      </li>
      <li>
        <a rel="me"
           href="https://live.xbox.com/en-US/Profile?gamertag=TheDoctorWhat">XBox Live</a>: TheDoctorWhat
      </li>
      <li>
        <a rel="me"
           href="http://us.playstation.com/publictrophy/index.htm?onlinename=docwhat">PSN</a>: docwhat
      </li>
    </ul>
  </li>
  <li>
    <h4>Development Egos</h4>
    <ul>
      <li>
        <a rel="me"
           href="http://github.com/docwhat">GitHub</a>
      </li>
      <li>
        <a rel="me"
           href="http://userscripts.org/users/57156">UserScripts</a>
      </li>
    </ul>
  </li>
  <li>
    <h4>Donations</h4>
    <ul>
      <li>
        <a rel="me"
           href="http://www.amazon.com/registry/wishlist/2940VECHO9PJY?reveal=unpurchased&filter=all&sort=priority&layout=standard">Amazon Wishlist</a>
      </li>
      <li>
  <a
      href="http://www.thinkgeek.com/brain/gimme.cgi?wid=81e27ae08">Think Geek Wishlist</a>
      </li>
      <li>
        <form action="https://www.paypal.com/cgi-bin/webscr" method="post" style="margin:0">
          <input type="hidden" name="cmd" value="_donations"/>
          <Input type="hidden" name="business" value="docwhat@gerf.org"/>
          <input type="hidden" name="lc" value="US"/>
          <input type="hidden" name="item_name" value="Gerf.Org"/>
          <input type="hidden" name="item_number" value="docwhat.org"/>
          <input type="hidden" name="currency_code" value="USD"/>
          <input type="hidden" name="bn" value="PP-DonationsBF:btn_donate_SM.gif:NonHostedGuest"/>
          <input type="image" src="https://www.paypal.com/en_US/i/btn/btn_donate_SM.gif" border="0" name="submit" alt="PayPal - The safer, easier way to pay online!" style="margin:0"/>
          <img alt="" border="0" src="//www.paypal.com/en_US/i/scr/pixel.gif" width="1" height="1"/>
        </form>
      </li>
      <li>
  <a href="http://flattr.com/thing/303004/The-Doctor-What" target="_blank">
<img src="//api.flattr.com/button/flattr-badge-large.png" alt="Flattr this" title="Flattr this" border="0" /></a>
      </li>
      <li style="display: none">
        <a rel="me" href="http://christian.holtje.org/">Vanity URL</a>
      </li>
    </ul>
  </li>
</ul>

<?php
/*
<!--Local Variables:-->
<!--mode: nxml-->
<!--End:-->
 */
?>
